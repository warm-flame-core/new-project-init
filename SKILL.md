...
description: 在新项目开工前、中途加入已有项目补建文档体系、或已有文档想完善规范并固化 AI 协作工作流时使用
---
...
**这是什么**：一个「项目文档体系初始化」skill——通过**提问驱动**把项目规划落实为一组特化规范文件（CLAUDE.md / memory 记忆库 / docs 文档 / specs 模块流程 / .gitignore 等），并固化「AI 分角色协作开发」工作流。覆盖**三种适用场景**：新项目开工前 / 中途加入已有项目补建体系 / 已有文档想完善规范。

**核心原则**：提问驱动落实（问透再写）· 先设计后动手 · 三种场景三分支（全新/中途/存量完善）· 双受众三因素 · on/off 开关 · 唯一出处 · 讨论驱动迭代 · 记忆库写入纪律 · 文档维护规则（变更记录必填）。
...
- **适合谁**：新项目开工前 / 中途加入已有项目补建体系 / 已有文档但想**完善规范 + 固化 AI 工作流**（三类场景都支持，见执行流程）。
...
2. **三种场景三分支**：全新项目（11 轮问询）/ 中途加入（7 轮）/ 存量完善（4 轮 + 限制规则）。存量完善核心是**谨慎**：进度以你口述为准（R1）、在途模块隔离不改（R2）、范围逐项确认（R5）、只记录不重构（R6）。
...
用户在新项目开工前、或中途加入项目补建体系时调用。你扮演「项目工程化教练 + 规划挖掘者」：**通过一轮又一轮提问**（用户不会嫌多），把用户内心对项目的规划全部挖出来，**转化为一组按项目特化的规范文件**：README + CLAUDE.md + memory（含 logs 与写入纪律）+ agents 角色 + docs 文档 + specs 模块流程 + .gitignore。
